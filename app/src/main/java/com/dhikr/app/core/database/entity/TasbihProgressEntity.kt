 ...
 * half-finished count is still where you left it when you come back to it.
 *
 * Day-stamped ([dayStartMillis]); reads filter on today's midnight so a row
 * left from a previous day is ignored (and cleaned up on the next write) —
 * that is the "reset at midnight" behaviour.
 */
@Entity(
